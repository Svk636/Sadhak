/* ═══════════════════════════════════════════════════════════════════
   ...
   Cache strategy:
     ...
     • Groq API / Anthropic API / other calls → Network-only (never cache)
   ═══════════════════════════════════════════════════════════════════ */

'use strict';

const CACHE_VERSION   = 'v5.1';
const SHELL_CACHE     = 'sadhak-shell-v5.1';
const FONT_CACHE      = 'sadhak-fonts-v5.1';
const CDN_CACHE       = 'sadhak-cdn-v5.1';
...
self.addEventListener('install', function (event) {
  ...
  event.waitUntil(
    caches.open(SHELL_CACHE).then(function (cache) {
      /* Cache each shell URL individually so a single missing icon never
         prevents index.html from being pre-cached. cache.addAll is
         atomic — one failure would cancel the entire batch. */
      return Promise.all(
        SHELL_URLS.map(function (url) {
          return cache.add(url).catch(function (err) {
            console.warn('[साधक SW] Failed to pre-cache:', url, err);
          });
        })
      );
    })
    /* NOTE: skipWaiting() is intentionally NOT called here.
       Calling it unconditionally in install would auto-reload every open
       tab the moment a new SW installs, interrupting active sessions and
       making the update banner dead code.
       skipWaiting() is only called when the user taps "Update" — via the
  ...
  return caches.open(cacheName).then(function (cache) {
    return cache.match(request).then(function (cached) {
      var networkFetch = fetch(request).then(function (response) {
        ...
        /* If network fails, return cached if available, else 503 */
